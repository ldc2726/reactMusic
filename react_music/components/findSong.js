...
import { SearchBar, Button, WhiteSpace,Carousel, WingBlank,  Tabs, Badge } from 'antd-mobile';
import {Link}  from 'react-router'
import '../scss/home/findSong.scss'
import host from '../jsPlug/host.js'
import $ from 'jquery'
class FindSong extends React.Component {
	constructor(props) {
		super(props)
		this.state = {
			data: [],
    ...
    seachChange(s){
		...
		$.ajax({
			type:"get",
			url:host+'/search?keywords='+s,
			success:function (data) {
				console.log(data['result']['songs'])
				_this.setState({seach_list:data['result']['songs']})
            }
		})
	}
	recom_songs(){ //推荐歌单
	...
	new_songs(){
		...
		$.ajax({
			type:"get",
			url:host+"/top/list?idx=1",
			success:function (data) {
				console.log(data)
                data = data['playlist']['tracks']
                _this.setState({new_song:data})
            }
		})
	}
	componentDidMount() {
		...
		$.ajax({
			...
			url:host+"/banner",
			success:function(data){
				_this.setState({data:data.banners})
			}
		});
		this.recom_songs();
		this.new_songs();
...
//		  { title: <Badge dot>Third Tab</Badge> },
		...
		return(
			<div id="findsong">
				<div id="findHead">
					<div className='head'>
						<span className='iconfont icon-luyin record'></span>
						<div className="seach"><SearchBar placeholder="Search" maxLength={8}  onChange={this.seachChange.bind(this)} onFocus={this.seachFocus.bind(this)} onBlur={this.seachCancel.bind(this)}/></div>
						<span className="iconfont icon-yuyinshibie player"></span>
					</div>
				</div>
				<Tabs tabs={tabs}
			      initialPage={0}
                                  ...
                                  this.state.new_song.map((item,index)=>(
                                      <li key={index}>
										  <Link to={'player/'+item.id}>
											  ...
											  <p className="singer_name"><em></em>{item['ar'][0]['name']}</p>
										  </Link>
